...
$meilleur=0;
  ...
  $slides = $bdd ->prepare('INSERT INTO partenaire(logo,activite,id_utilisateurs,telephone,mobile,id_commune,web,map,localisation,id_categories,special,description,reduction,facebook,whatsapp,instagramm,youtube,confirme,meilleur) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
                  $slides->execute(array( 
                        $logo,$activite,$session,$telephone,$mobile,$commune,$web,$map,$localisation,$categorie,$special,$description,$reduction,$facebook,$whatsapp,$instagramm,$youtube,$confirmer,$meilleur
                      ));
  ...
  $msg= "message enregistre";
                  $message ="Vos informations n'ont pas été bien envoyer";
}
...
$session=$c['id']; 
  ...
  $slides = $bdd ->prepare('UPDATE partenaire SET logo = ? WHERE id= ?');
                  $slides->execute(array( 
                        $chemin,$session
  ...
  $insertfonction = $bdd->prepare("UPDATE membres SET fonction = ? WHERE id = ?");
...
$da ->execute(array($fini,$sesson,$categorie));
   }else{
    echo "string";
   }
  ...
  <div class="collapse navbar-collapse menu" id="navbarSupportedContent" style="font-family: 'Montserrat', sans-serif;">
    <ul class="menu navbar-nav mr-auto">
      <li class="nav-item active" >
        <a class="nav-link" style="color:#FF7D00" href="">Espace publicitaire</a>
      </li>
      <li class="nav-item" >
...
<div class="tab" style="margin-top: 10%;">
  <button class="tablinks" onclick="openCity(event, 'Londo')" id="defaultOpen">Ajouter / modifier nom partenaire</button>
...
<div id="Londo" class="tabcontent" style="margin-top: 10%;">

  <div class="row">
    <div class="col-lg-6">
      <div class="container" style=" width: 350px; background-color: grey;">
  
      <form method="POST" action="#" style="margin-top: 50%;">
      ...
      <input class="form-control" type="text" name="activite" placeholder="Entrez le nom du partenaire" />
             ...
             <button type="submit" name="nom" class="btn btn-primary">Enregistrer </button>
      </form>
</div>
</div>
...
$c = $commentaires->fetch();
      ...
      <h2 style="background-color: grey">Nom de l'entreprise</h2>
      <h5 style=""><?php echo $c['activite'] ?></h5>
...
<div id="London" class="tabcontent" style="margin-top: 10%;">
  ...
  <div class="row">
    <div class="col-lg-6">
      ...
      <form method="POST" action="profil.php?id=<?php echo $userinfo['id'] ?>" enctype="multipart/form-data">
    <div class="form-group text-center">
          <img src="../administration/profil/assets/img/find_user.png" onclick="triggerClick()" id="profileDisplay" style="width: 50%;" />
          <label for="profileImage" style="color: white; font-size: 19px;">cliquer pour ajouter votre logo</label>
