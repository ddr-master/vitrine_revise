 ...
 while ($userinfo = $con->fetch()){
  ...
  if (isset($_GET['confirme']) AND !empty($_GET['confirme'])) {
    $confirmer = (int) $_GET['confirme'];
     ...
     $req = $bdd->prepare('UPDATE categories SET confirme = 1 WHERE id = ?');
  ...
  if (isset($_GET['confirm']) AND !empty($_GET['confirm'])) {
    $confirmer = (int) $_GET['confirm'];
     ...
     $req = $bdd->prepare('UPDATE categories SET confirme = 0 WHERE id = ?');
    $req->execute(array($confirmer));
    ...
    $message ="Bien ajouter dans la catgories meilleurs partenaire";
           ...
           ?>
                                        ...
                                        <tr class="odd gradeX">
                                            ...
                                            <td><?php echo $userinfo['code']; ?></td>
                                            <td>
                                              <?php 
                                              $confirme= $userinfo['confirme'];
                                                  if ($confirme == 0) { ?>
                                                   <a href="suprimer_modifier_categorie.php?confirme=<?= $userinfo['id']; ?>"type="button" class="btn btn-success">Non</a>
                                                 <?php  }else{  ?>
<a href="suprimer_modifier_categorie.php?confirm=<?= $userinfo['id']; ?>"type="button" class="btn btn-danger">Enlever</a>
                                                  <?php }  ?>
                                            ...
                                            <td><a href="traitement/modifier_categories.php?id=<?= $userinfo['id']; ?>"type="button" class="btn btn-primary">Modifier</a></td>

                                            <td><a href="suprimer_modifier_categorie.php?supprimer=<?= $userinfo['id']; ?>" type="button" class="btn btn-danger">Supprimer</a></td>
                                        </tr>
